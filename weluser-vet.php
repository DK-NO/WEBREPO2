...
<div class="container-fluid contenedor-nav">
	...
	//si funciona solo tenemos que cambiar la tabla cuando cambiemos a la tabla mascotas
        ...
        $data = mysqli_query($link, "SELECT * FROM veterinaria WHERE estado_vet='$est'")
				or die("no hay veterinarias activas");
			...
			while ($var = mysqli_fetch_array($data)) 
			{
                ...
                echo "<h2>".$var['nombre_vet']."</h2>";
                
                echo "<h7>".$var['correo_vet']."</h7>";
                echo "<h5>".$var['cod_cel_vet']." ".$var['cel_vet']."</h5>";
                echo "<h5>".$var['city_vet']."</h5>";
                echo "<h7>".$var['ubicacion_vet']."</h7>";
                echo "</div>";
...
<a href="weluser-vet.php"><h3>Actualizar?</h3></a>
        </div>
